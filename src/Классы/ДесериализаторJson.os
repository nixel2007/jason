...
Функция ПреобразоватьОбъектДесериализации(ДесериализованныйОбъект, ТипОбъекта, ТипЭлементаМассива = Неопределено)
	...
	ИначеЕсли ТипОбъекта = Тип("Массив") Тогда
		Возврат ПреобразоватьМассив(ДесериализованныйОбъект, ТипОбъекта, ТипЭлементаМассива);
...
Функция ПреобразоватьМассив(Объект, ТипОбъекта, ТипЭлементаМассива = Неопределено)
	
	Результат = Новый Массив;

	Для Каждого Элемент Из Объект Цикл
		Если ТипЭлементаМассива <> Неопределено Тогда
			ПреобразованныйЭлемент = ПреобразоватьОбъектДесериализации(Элемент, ТипЭлементаМассива);
		Иначе
...
Функция ПреобразоватьПользовательскийТип(Объект, ТипОбъекта)
	...
	Для Каждого КлючИЗначение Из Объект Цикл
		...
		ТипЭлементаМассива = Неопределено;
		Если АннотацияТип <> Неопределено Тогда
			...
			// Если это массив, проверяем есть ли аннотация &ДляКаждого для типа элементов
			Если ТипСвойства = Тип("Массив") Тогда
				АннотацияДляКаждого = РаботаСАннотациями.ПолучитьАннотацию(Свойство, "ДляКаждого");
				Если АннотацияДляКаждого <> Неопределено Тогда
					// Ищем следующую аннотацию &Тип которая определяет тип элементов
					АннотацииСвойства = РаботаСАннотациями.ПолучитьАннотации(Свойство);
					НайденаАннотацияДляКаждого = Ложь;
					Для Каждого АннотацияСв Из АннотацииСвойства Цикл
						Если НайденаАннотацияДляКаждого И АннотацияСв.Имя = "Тип" Тогда
							ИмяТипаЭлемента = РаботаСАннотациями.ПолучитьЗначениеПараметраАннотации(АннотацияСв, "Значение");
							ТипЭлементаМассива = Тип(ИмяТипаЭлемента);
							Прервать;
						КонецЕсли;
						Если АннотацияСв.Имя = "ДляКаждого" Тогда
							НайденаАннотацияДляКаждого = Истина;
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
		...
		ЗначениеСвойства = ПреобразоватьОбъектДесериализации(ЗначениеСвойства, ТипСвойства, ТипЭлементаМассива);

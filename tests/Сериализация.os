...
&Тест
Процедура СериализацияТолькоПолейСМодификаторомЭкспорт() Экспорт

	// Дано
	// Тестовый класс с полями как с модификатором Экспорт, так и без него
	// В OneScript только поля с модификатором Экспорт доступны через рефлексию и могут быть сериализованы
	...
	// Проверяем, что поля БЕЗ модификатора Экспорт НЕ сериализуются
	// (так как они не доступны через рефлексию в OneScript)
	Ожидаем.Что(ПрочитанныйОбъект.Свойство("ПолеБезЭкспорта")).Равно(Ложь);
	Ожидаем.Что(ПрочитанныйОбъект.Свойство("renamed_field_no_export")).Равно(Ложь);

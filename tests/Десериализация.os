...
&Тест
Процедура ДесериализацияМассиваПользовательскихКлассов() Экспорт

	// Дано
	JSON = "
	|{
	|  ""Элементы"": [
	|    { ""Имя"": ""Объект1"", ""Значение"": ""Значение1"" },
	|    { ""Имя"": ""Объект2"", ""Значение"": ""Значение2"" }
	|  ]
	|}";
	Десериализатор = Новый ДесериализаторJson();

	// Когда
	Объект = Десериализатор.Десериализовать(JSON, Тип("КонтейнерМассиваОбъектов"), Ложь);

	// Тогда
	Ожидаем.Что(Объект).ИмеетТип("КонтейнерМассиваОбъектов");
	Ожидаем.Что(Объект.Элементы).ИмеетТип("Массив");
	Ожидаем.Что(Объект.Элементы).ИмеетДлину(2);
	
	// Проверяем первый элемент массива
	Ожидаем.Что(Объект.Элементы[0]).ИмеетТип("МойКласс2");
	Ожидаем.Что(Объект.Элементы[0].Имя).Равно("Объект1");
	Ожидаем.Что(Объект.Элементы[0].Значение).Равно("Значение1");
	
	// Проверяем второй элемент массива
	Ожидаем.Что(Объект.Элементы[1]).ИмеетТип("МойКласс2");
	Ожидаем.Что(Объект.Элементы[1].Имя).Равно("Объект2");
	Ожидаем.Что(Объект.Элементы[1].Значение).Равно("Значение2");
